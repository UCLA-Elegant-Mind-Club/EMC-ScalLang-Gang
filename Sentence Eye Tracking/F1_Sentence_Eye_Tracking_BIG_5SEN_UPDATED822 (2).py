...
if recordData:
    ...
    expName = 'F1_Sentence_Eye_Tracking_BIG'
...
def instructions2():
    genDisplay({'text': 'You will see a white dot around the screen.',\
        'xPos': 0, 'yPos': centerY+4, 'heightCm': 3, 'color': 'white'}).draw()
    genDisplay({'text': 'When it appears, look at it until it disappears.',\
        'xPos': 0, 'yPos': centerY, 'heightCm': 3, 'color': 'white'}).draw()
...
if mistakes > 0:
    genDisplay({'text': 'These trials are a make-up of your mistakes',\
        'xPos': 0, 'yPos': centerY+6, 'heightCm': 2, 'color': 'white'}).draw()
    ...
    genDisplay({'text': 'and press Space to continue',\
        'xPos': 0, 'yPos': centerY+2, 'heightCm': 2, 'color': 'white'}).draw()
    win.flip()
